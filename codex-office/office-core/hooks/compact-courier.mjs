 ...
 */
import { existsSync, readFileSync, unlinkSync, appendFileSync, mkdirSync, rmdirSync, statSync, accessSync, constants } from "node:fs";
...
const run = async () => {
  ...
  try {
    ...
    let age = 0;
    try { age = Date.now() - statSync(candidate).mtimeMs; } catch { age = LOCK_STALE_MS + 1; }
    if (age < LOCK_STALE_MS) return;      // a live courier already owns this session
    lock = candidate;                     // stale: adopt it and clean up on the way out
  }
  ...
  const res = herdr(["agent", "prompt", name, prompt]);
  if (!res || res.error) return;          // delivery failed: request stays, retry next boundary
