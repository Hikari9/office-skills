...
**Record the returned agent ID/name.** Resume is the default — every subsequent round goes back to that same agent via SendMessage so it keeps the full history of what it already flagged and accepted, and a fresh reviewer per round re-litigates settled findings and cannot tell you whether round 2 regressed round 1. But resume is not free at accumulated size, and by round 4 it may no longer be the cheaper option: weigh measured cost, round count, and relatedness, and if you go fresh, hand the new agent a written digest instead of the transcript. **Re-verify every carried finding against `HEAD` before it goes in that digest** — a finding from an earlier verdict is a claim about the tree as it was then, and a later task may already have closed it. Observed 2026-09-11: two "open" minors recovered from an older verdict had both been fixed two tasks earlier, and the brief built on them would have had an executor revert landed work; plan-review caught it before dispatch. A digest is a fresh set of assertions, so it carries the same evidence burden as any other. Full rubric: [`../office-core/protocol/review-states.md`](../office-core/protocol/review-states.md#resume-vs-fresh--a-cost-decision-not-a-default). Declare `resume` or `fresh` and the reason in the round line either way.
...
**Aim the control at the deliverable, not at one of its attributes.** The natural mutation is a detail — a constant, a colour, a branch — and a suite can discriminate on every detail while never asserting the thing exists. Observed 2026-09-11: two tasks passed lint, types, build and a full suite, and deleting the feature itself (both charts, from the page) left every test green, because the data pipeline was exhaustively tested and the rendered output was not asserted at all. Before any attribute mutation, run the crude one: **remove the deliverable and confirm something fails.** Two corollaries. Where a stub or mock stands between the test and the artifact, the mutation must target what the test can actually observe — mutating the canvas dataset proved nothing, because the renderer was stubbed and only the legend swatch was visible. And where a fix is deliberately redundant, **no single-layer mutation can go red**: read the committed diff, count the mechanisms, and remove all of them — mutating from the brief's *description* of a four-layer fix produced a false alarm that cost a round to retract.
