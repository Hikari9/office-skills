...
43. **A low-effort reviewer at a stronger model can outperform a stronger-effort executor at a weaker one.** `sonnet` high executor + `opus` low reviewer, small backend guard change: the reviewer caught an unconditional, non-fail-soft dependency added to a hot path that the executor introduced and missed — on a 5xx it would have zeroed output for a class of records the change was explicitly scoped to leave alone. `opus` low was worth its cost as the gate, independent of the executor's own effort tier. → `auto-routing` (reviewer floor is not the executor's tier)
