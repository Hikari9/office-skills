...
second telemetry format. Keep the existing columns and append these compact
...
one. `fallback_reason` explains why the preferred triple was unusable; `attempts`
keeps the dedicated fallback path auditable. Existing rows without these fields
remain valid historical compatibility rows. The normal event fields
`brand`, `model`, `effort`, and `dispatch_form` still describe the actual call;
executor and reviewer routing are not rewritten by planner selection.
