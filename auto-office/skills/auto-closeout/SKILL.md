...
11. **Report facts, not implied success.** Seven rows, written into the target repo, not this plugin:
    ...
    compact v2 planner line `planner_mode · orchestrator=<harness/model@effort> ·
    planner=<harness/model@effort> · reused_from_orchestrator=<true|false> ·
    fallback_used=<true|false> · fallback_reason=<reason|null>`; attempted triples and failure reasons
    remain in the serialized handoff or run artifact.
