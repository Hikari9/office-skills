...
| `state` flips to `busy`/`working` right after your text send, but the agent never acts on the message and later reports work you did not ask for | **`busy` after a send is NOT proof of submission.** The agent was still finishing its *previous* turn while your text sat unsubmitted in the box. Verified 2026-08-30: a planner read `busy` as delivery, and the agent went on to hand back the task as complete with the review-round instructions never processed — costing a full round-trip and a near-miss on a production merge | Never treat a state transition alone as delivery. **Strip the log and confirm the input box is EMPTY** (your text no longer rendered between the prompt separator lines) before concluding the send landed. If the text is still visible, send a bare `\r`. For anything longer than one sentence, prefer writing the instructions to a file in the worktree and sending a one-line pointer to it — a file cannot be half-delivered |
