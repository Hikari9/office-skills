...
| Every complete reviewer verdict is copied to the existing PR before triage or follow-up; every `CHANGES REQUIRED` finding is copied verbatim, followed by a finding-by-finding resolution comment | core | `office-core/protocol/review-states.md` (The PR review record) | All three `review-gate.md` files and reviewer spokes | User-directed review-record lifecycle change |
