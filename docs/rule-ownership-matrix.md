...
| Tracking issue is identified before plan approval and the draft PR body carries the issue plus an immutable plan blob deeplink | core | `office-core/protocol/plan-contract.md` (Run metadata and tracking issue) | All three planning/executor routes | User-directed core 6.0.0 lifecycle change |
...
| Dispatch surface: `HERDR_ENV=1` uses visible Herdr panes (direct children right, nested children below) and never in-session; absent Herdr preserves each office's normal routing | core | `office-core/protocol/roles-and-authority.md`; `office-core/skills/herdr/SKILL.md` | All three hubs and executor spokes | New shared override |
...
| Planner-held by default: ready-for-review, plan removal, merges, deploys, migrations, remote/DNS/config, credentials, outbound messages; executor bootstrap is the named exception for push, draft PR, and the two executor-event comments | core | `office-core/protocol/roles-and-authority.md` (Blast-radius ceiling and Executor-owned draft-PR bootstrap); `office-core/protocol/executor-bootstrap.md` | All three executor spokes and briefs; all three closeout spokes; all three hubs | User-directed core lifecycle change |
| Executor bootstrap: plan-only first commit, named-branch push, one draft PR, approved-plan/execution-begins comment, first-executor-completion comment, fail-closed verification | core | `office-core/protocol/executor-bootstrap.md` | `codex-office/skills/codex-executor/SKILL.md`, `agy-office/skills/agy-executor/SKILL.md`, `agy-office/references/executor-brief.md`, `auto-office/skills/auto-loop/SKILL.md` | User-directed core lifecycle change |
| Milestones are committed and recorded in local run state; final closeout posts the approval summary, removes the plan, marks ready, and merges | core | `office-core/protocol/plan-contract.md`, `office-core/protocol/closeout.md` | All three hubs and closeout spokes; `auto-office/skills/auto-loop/SKILL.md` | User-directed core lifecycle change |
...
separate `references/routing.md`. Outside Herdr, same-brand Codex workers use in-session subagents; cross-brand
...
| Only three PR comments: approved-plan/execution-begins, first-executor-completion, and final `APPROVED` summary; intermediate verdicts and fix resolutions stay in run artifacts | core | `office-core/protocol/review-states.md` (PR comment policy) | All three `review-gate.md` files and reviewer spokes | User-directed review-record lifecycle change |
