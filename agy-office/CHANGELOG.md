...
- Re-vendored from core `10.0.0`; no agy-office-specific text changed.

## 8.0.0 — 2026-09-01

Core `9.0.0`.

- Adds the shared Herdr dispatch override: when `HERDR_ENV=1`, delegated agy workers, verifiers,
  scouts, and reviewers run in visible Herdr panes, nested children go below their parent,
  in-session subagents are disabled, and created panes close after settled results are read.
- Preserves the existing agy CLI/subagent routing when Herdr is not detected.
