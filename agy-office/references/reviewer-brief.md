...
- **Gate output, captured by the planner** (not by the executor): `<path or inline>`
- **Planner's verification pass (Phase 2b), one line per check:** `<what was independently verified, and what is being passed through to you unverified>`
- **Live surfaces written by this run:** `<each deployed artifact — page/block/table/endpoint — or "none">`
- **Decisions made under uncertainty** (the handoff's `## Upline` `[decided]` list, plus whether it looked complete against the diff):
...
**2. Interface truth — check this before anything else, and check it in the source.** For every hook, action, filter, callback, event handler, SDK method, or HTTP endpoint the diff registers, calls, or tests:

- Open the **real definition** in the repo or the vendored dependency and read the actual signature. Do not accept the handoff's "Interfaces verified" table as evidence; it is the executor's claim about its own work. Use it as a checklist, then verify each row.
- Compare argument count, order, types, and return contract against how the diff uses it.
- Check the **tests** against the real signature too. A test asserting an invented shape is a finding in its own right, graded **Critical**, even when it passes — it will keep passing while production is broken.
- A diff that touches interfaces with an **empty or absent** table means the executor skipped the step it was told to do. Verify everything yourself and say so in your verdict.

Any mismatch here is **Critical**, independent of how clean the rest of the code is.

**3. Correctness.** Read for defects, not for style: edge cases, null/empty/boundary inputs, error paths, off-by-one, race conditions, unhandled rejections, incorrect state transitions, security-relevant handling of input and secrets. For each defect give a **concrete failure scenario** — specific inputs or state → the wrong output or crash. A finding you cannot make concrete is a question, not a finding; ask it as such.

**4. Guard width.** This executor scopes conditionals conservatively. For every conditional, guard, capability check, or asset enqueue the diff added or changed, compare its scope against what the plan asked for. A guard narrower than the spec fails silently — nothing errors, the feature simply does not appear where it should — so it will not show up in any test output.

**5. Scope containment.** The executor ran unsandboxed with no approval stop, so verify it stayed inside its guardrails rather than assuming it did. Check the diff and the repo for: files touched under the plan's protected/out-of-scope paths, untracked or added files no work item asked for (cross-check the handoff's "Files created that are not in the work items" section — and treat a stale section as a finding), work on `PLANNER-HELD` tasks it was told not to run, commits on a branch other than the designated one, and any push, PR, deploy, remote-config change, message send, or credential access the prompt did not explicitly authorize (`git log --oneline @{u}..` and `git remote -v` settle most of it). Any of these is **Critical**.

**6. Quality.** Tests that actually assert behavior rather than existing; no duplicated logic blocks; no magic numbers; naming and conventions matching surrounding code; no dead code left behind.

**7. Evidence gate — this one is absolute.**

> **You may not return APPROVED without seeing actual command output for the validation commands.**

Use the **planner-captured** gate output listed in your inputs. The executor's own pasted output is not evidence in this office — it cannot be attributed to this HEAD and it comes from an agent whose exit code means nothing. If the planner's output is missing, does not match the commands you were given, or predates HEAD:
...
A green lint is not a green build. If the plan's gate includes a build, a lint run does not satisfy it. And a green suite does not satisfy check 2 — see above for why.

**8. Live-write evidence — a read-back, not an exit code.** For every surface listed under "Live surfaces written by this run", a writer reporting success proves nothing: the write can land on the wrong row, a cache can keep serving the previous version, or the platform can accept the call and ignore it. Demand, for each one:
...
2. **Do the interface check (2) next, before reading for general correctness.** It is the highest-yield pass on this executor's output and it changes how you read everything downstream: if a signature is wrong, the tests around it are worthless as signal.
3. Walk the diff file by file. Open the surrounding source in the repo whenever the diff alone cannot tell you whether something is correct; a diff hides its own callers.
4. Check the handoff's "Deviations", "Files created that are not in the work items" and "Upline" sections. Triage each: does it block merge, or not? Say so explicitly for each one — an unexamined deferral is how real defects ship. For each `[decided]` entry, verify the choice was *correct*, not just defensible, and say which you concluded.
...
Interfaces checked: <per interface: real signature at file:line → matches | MISMATCH (see finding N)>
Guard width: <per changed conditional: matches spec | narrower (see finding N)>
Scope containment: <clean — no protected paths, unrequested files, unauthorized side effects, or PLANNER-HELD work | violations (see finding N)>
Deferrals triaged: <each deferred item → blocks merge | acceptable>
Upline decisions checked: <each [decided] entry → correct | wrong (see finding N)>
Gate evidence: <verified: `<command>` output seen, planner-captured | MISSING>
